            ...
            try:
                ...
                if(len(byte_stream) >= 9):
                    for i in range(1,len(byte_stream)):
                        if(i+9 < len(byte_stream) and byte_stream[i:i+9] == "201502060"):
                            Nbyte_stream = byte_stream[0:i]+byte_stream[i+9:]
                            #display(Nbyte_stream)
                            Nbyte_stream = Nbyte_stream.encode("utf-8")
                            #print(Nbyte_stream)
                            sound_stream = list()
                            sound_stream.append(HANDSHAKE_START_HZ)
                            for i in Nbyte_stream:
                                j = i >> 4
                                sound_stream.append((j&0x0f)*STEP_HZ+START_HZ)
                                sound_stream.append((i&0x0f)*STEP_HZ+START_HZ)
                            sound_stream.append(HANDSHAKE_END_HZ)
                            sound_np = np.array(sound_stream)
                            print(sound_np)
                            p = pyaudio.PyAudio()
                            stream = p.open(format=pyaudio.paFloat32,channels=1,rate=44100,output=True)
                            for i in sound_np:
                                sample = np.sin(2*np.pi*np.arange(44100)*i/44100).astype(np.float32)  
                                stream.write(sample)                            
